...
public class OrderServiceBean implements OrderService {
    ...
    public OrderAck submitOrder(Order order) {
        ...
        try {
            ...
            try {
                Thread.sleep( Util.getRandomDelay()) ;
            } catch (InterruptedException e) {
                e.printStackTrace();
            }

            _funds.checkfunds(new FundCheck(orderItem,order));
            ...
            orderAck.setAccepted(false).setStatus("InsufficientFunds Funds");
    ...
    public Receipt makePayment(Payment payment) {
        ...
        _funds.makePayment(payment) ;
        return (ret);
    }
    private  boolean checkFund(Order order) throws  InsufficientFundsException{
        if(order.getCustomer().equalsIgnoreCase("Fred")){
            throw  new InsufficientFundsException("Fred has not enough funds to purchase", order.getCustomer());
        }
       return true;
    }
}
